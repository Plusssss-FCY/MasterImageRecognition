...
def Extend(path):
    ...
    cv2.putText(origin, 'Major_Axis : ' + str(Real_Major_Axis_Length), (280, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                (0, 0, 155), 2, cv2.LINE_AA)
    cv2.putText(origin, 'Minor_Axis : ' + str(Real_Minor_Axis_Length), (280, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                (0, 0, 155), 2, cv2.LINE_AA)
    cv2.putText(origin, 'P_extend: ' + str(P_extend), (280, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 155), 2,
                cv2.LINE_AA)

    # 显示
    # cv2.namedWindow('p-extend', cv2.WINDOW_AUTOSIZE)
    # cv2.imshow('p-extend', origin)
    cv2.imwrite('picture/p-extend.png', origin)

    # img = mpimg.imread('picture\\p-extend.png')
    # plt.figure()
    # fontSet = FontProperties(fname=r"C:\Windows\Fonts\stsong.ttf", size=16)
    # plt.title(u"p-extend", color='k', fontproperties=fontSet, weight='heavy')
    # plt.imshow(img)
