...
def Spherical(path):
    ...
    cv2.putText(origin, 'min_radius : ' + str(min_radius), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (50, 50, 50), 2,
                cv2.LINE_AA)
    cv2.putText(origin, 'max_radius: ' + str(max_radius), (50, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (50, 50, 50), 2,
                cv2.LINE_AA)
    cv2.putText(origin, 'P_pherical: ' + str(P_pherical), (50, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (50, 50, 50), 2,
                cv2.LINE_AA)

    #显示
    # cv2.namedWindow('p-pherical', cv2.WINDOW_AUTOSIZE)
    # cv2.imshow('p-pherical', origin)
    cv2.imwrite('picture/p-pherical.png', origin)

    # img = mpimg.imread('picture\\p-pherical.png')
    # plt.figure()
    # fontSet = FontProperties(fname=r"C:\Windows\Fonts\stsong.ttf", size=16)
    # plt.title(u"p-pherical", color='k', fontproperties=fontSet, weight='heavy')
    # plt.imshow(img)
