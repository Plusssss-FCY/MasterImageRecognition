...
def Circle(path):
    ...
    cv2.putText(origin, 'S_maxContour : ' + str(largest_area), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (50, 50, 50), 2,
                cv2.LINE_AA)
    cv2.putText(origin, 'Major_Axis: ' + str(Real_Major_Axis_Length), (50, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                (50, 50, 50), 2, cv2.LINE_AA)
    cv2.putText(origin, 'P_circle: ' + str(P_circle), (50, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (50, 50, 50), 2,
                cv2.LINE_AA)

    # 显示
    # cv2.namedWindow('p-circle', cv2.WINDOW_AUTOSIZE)
    # cv2.imshow('p-circle', origin)
    cv2.imwrite('picture/p-circle.png', origin)

    # img = mpimg.imread('picture\\p-circle.png')
    # plt.figure()
    # fontSet = FontProperties(fname=r"C:\Windows\Fonts\stsong.ttf", size=16)
    # plt.title(u"p-circle", color='k', fontproperties=fontSet, weight='heavy')
    # plt.imshow(img)
