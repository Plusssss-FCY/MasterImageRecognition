...
def Leaf(path):
    ...
    cv2.putText(origin, 'min_radius : ' + str(min_radius), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (50, 50, 50), 2,
                cv2.LINE_AA)
    cv2.putText(origin, 'Major_Axis_Length: ' + str(Real_Major_Axis_Length), (50, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                (50, 50, 50), 2, cv2.LINE_AA)
    cv2.putText(origin, 'P_leaf: ' + str(P_leaf), (50, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (50, 50, 50), 2,
                cv2.LINE_AA)

    #显示
    # cv2.namedWindow('p-leaf', cv2.WINDOW_AUTOSIZE)
    # cv2.imshow('p-leaf', origin)
    cv2.imwrite('picture/p-leaf.png', origin)

    # img = mpimg.imread('picture\\p-leaf.png')
    # plt.figure()
    # fontSet = FontProperties(fname=r"C:\Windows\Fonts\stsong.ttf", size=16)
    # plt.title(u"p-leaf", color='k', fontproperties=fontSet, weight='heavy')
    # plt.imshow(img)
